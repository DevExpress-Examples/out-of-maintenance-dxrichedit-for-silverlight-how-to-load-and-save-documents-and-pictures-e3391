...
{\b\ul\fs96\cf5 or}{\b\fs96\cf5 mat}{\b\fs96\cf0 ting in one }{\fs96\cf0 paragraph.}\par}";
        ...
        {
            #region #saveimage
            ReadOnlyDocumentImageCollection imgs = richEditControl1.Document.Images.Get(richEditControl1.Document.Selection);
            if (imgs.Count > 0) {
                MemoryStream ms = new MemoryStream(imgs[0].Image.GetImageBytesSafe(OfficeImageFormat.Png));
                SaveFileDialog sfdlg = new SaveFileDialog();
